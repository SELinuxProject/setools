...
@pytest.fixture
def widget(mock_query, request: pytest.FixtureRequest, qtbot: QtBot) -> PermissionCriteriaWidget:
    """Pytest fixture to set up the widget."""
    marker = request.node.get_closest_marker("obj_args")
    kwargs = marker.kwargs if marker else {}
    w = PermissionCriteriaWidget(request.node.name, mock_query, "name", **kwargs)
    qtbot.addWidget(w)
    w.show()
    return w


def test_base_settings(widget: PermissionCriteriaWidget) -> None:
    """Test base properties of widget."""
    assert widget.perm_model.item_list == ["bar_perm1", "bar_perm2", "common_perm", "foo_perm1",
                                           "foo_perm2"]


def test_set_classes(widget: PermissionCriteriaWidget, mock_query) -> None:
