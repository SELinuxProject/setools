...
import setools
from setoolsgui.widgets.typequery import TypeQueryTab


@pytest.fixture
def widget(mock_policy, request: pytest.FixtureRequest, qtbot: QtBot) -> TypeQueryTab:
    """Pytest fixture to set up the widget."""
    marker = request.node.get_closest_marker("obj_args")
    kwargs = marker.kwargs if marker else {}
    w = TypeQueryTab(mock_policy, **kwargs)
    qtbot.addWidget(w)
    w.show()
    return w


def test_docs(widget: TypeQueryTab) -> None:
...
def test_layout(widget: TypeQueryTab) -> None:
    """Test the layout of the criteria frame."""
    name, attrs, permissive = widget.criteria
    ...
    assert widget.criteria_frame_layout.itemAtPosition(0, 1).widget() == permissive
    assert widget.criteria_frame_layout.itemAtPosition(1, 0).widget() == attrs
...
def test_criteria_mapping(widget: TypeQueryTab) -> None:
    """Test that widgets save to the correct query fields."""
    name, attrs, permissive = widget.criteria

    assert isinstance(widget.query, setools.TypeQuery)
    assert name.attrname == "name"
    assert attrs.attrname == "attrs"
    assert permissive.attrname == "permissive"
