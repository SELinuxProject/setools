...
import setools
from setoolsgui.widgets.netifconquery import NetifconQueryTab
from setoolsgui.widgets import criteria


@pytest.fixture
def widget(mock_policy, request: pytest.FixtureRequest, qtbot: QtBot) -> NetifconQueryTab:
...
def test_docs(widget: NetifconQueryTab) -> None:
...
def test_layout(widget: NetifconQueryTab) -> None:
...
def test_criteria_mapping(widget: NetifconQueryTab) -> None:
    """Test that widgets save to the correct query fields."""
    name, context = widget.criteria
    context = typing.cast(criteria.ContextMatch, context)

    assert isinstance(widget.query, setools.NetifconQuery)
