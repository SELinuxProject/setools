...
import setools
from setoolsgui.widgets.typeattrquery import TypeAttributeQueryTab


@pytest.fixture
def widget(mock_policy, request: pytest.FixtureRequest, qtbot: QtBot) -> TypeAttributeQueryTab:
    """Pytest fixture to set up the widget."""
    marker = request.node.get_closest_marker("obj_args")
    kwargs = marker.kwargs if marker else {}
    w = TypeAttributeQueryTab(mock_policy, **kwargs)
    qtbot.addWidget(w)
    w.show()
    return w


def test_docs(widget: TypeAttributeQueryTab) -> None:
...
def test_layout(widget: TypeAttributeQueryTab) -> None:
    """Test the layout of the criteria frame."""
    name, types = widget.criteria
    ...
    assert widget.criteria_frame_layout.itemAtPosition(0, 1).widget() == types
...
def test_criteria_mapping(widget: TypeAttributeQueryTab) -> None:
    """Test that widgets save to the correct query fields."""
    name, types = widget.criteria

    assert isinstance(widget.query, setools.TypeAttributeQuery)
    assert name.attrname == "name"
    assert types.attrname == "types"
