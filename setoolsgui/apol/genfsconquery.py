...
class GenfsconQueryTab(SEToolsWidget, QScrollArea):
    ...
    def __init__(self, parent, policy, perm_map):
        ...
        self.policy = policy
        self.query = GenfsconQuery(policy)
        self.setupUi()
    ...
    def setupUi(self):
        ...
        # set up results
        self.table_results_model = GenfsconTableModel(self)
        ...
        logging.getLogger("setools.genfsconquery").addHandler(self.handler)
        ...
        self.path.textEdited.connect(self.clear_path_error)
        self.path.editingFinished.connect(self.set_path)
        self.path_regex.toggled.connect(self.set_path_regex)
    ...
    def update_complete(self, count):
        self.log.info("{0} genfscon(s) found.".format(count))
