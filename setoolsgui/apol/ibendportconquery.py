...
class IbendportconQueryTab(AnalysisTab):
    ...
    def __init__(self, parent, policy, perm_map):
        ...
        self.policy = policy
        self.query = IbendportconQuery(policy)
        self.setupUi()
    ...
    def setupUi(self):
        self.load_ui("apol/ibendportconquery.ui")
        ...
        # set up results
        self.table_results_model = IbendportconTableModel(self)
        ...
        logging.getLogger("setools.ibendportconquery").addHandler(self.handler)
        ...
        self.port.textEdited.connect(self.clear_port_error)
        self.port.editingFinished.connect(self.set_port)
    ...
    # Port criteria
    #
    def clear_port_error(self):
        self.clear_criteria_error(self.port, "Match the endport.")

    def set_port(self):
        try:
            self.query.port = self.port.text()
        except Exception as ex:
            self.log.error("Endport error: {0}".format(ex))
            self.set_criteria_error(self.port, ex)
    ...
    def save(self):
        ...
        save_lineedits(self, settings, ["name", "port", "user", "role", "type_",
                                        "range_"])
    ...
    def load(self, settings):
        load_checkboxes(self, settings, ["criteria_expander", "notes_expander", "name_regex",
                                         "user_regex", "role_regex", "type_regex", "range_exact",
                                         "range_overlap", "range_subset", "range_superset"])
        load_lineedits(self, settings, ["name", "port", "user", "role", "type_",
                                        "range_"])
