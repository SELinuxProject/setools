...
# Checked by default:
DEFAULT_CHECKED: typing.Final[tuple[str, ...]] = ("typebounds",)

__all__ = ('BoundsRuleType',)


class BoundsRuleType(CheckboxSetWidget):

    """
    Criteria selection widget presenting bounds rule types as a series
    of checkboxes.  The selected checkboxes are then merged into a single Python
    list consisting of object names (bounds ruletypes) and stored in the query's
    specified attribute.
    """
    ...
    def __init__(self, title: str, query: setools.PolicyQuery, attrname: str = "ruletype",
        ...
        super().__init__(title, query, attrname, (rt.name for rt in setools.BoundsRuletype),
...
if __name__ == '__main__':
    ...
    q = setools.BoundsQuery(setools.SELinuxPolicy())
